...
# --- 1. GLOBAL AUDIT DATA (Verified for Line 10 Stability) ---
AUDIT_DATA = {
    "Architecture": {
        ...
        "df": pd.DataFrame({
            ...
            "Significance": ["Addressable space.", "Land pattern design.", "Interleaving efficiency.", "Trace offset matching."]
        })
    },
    "DC Power": {
...
SOLUTIONS = {
    "Thermal Risk": "Implement BIOS-level 'Fine Granularity Refresh' to scale tREFI to 3.9us automatically when T-Case exceeds 85C.",
    ...
    try:
        ...
        for i, (key, tab) in enumerate(zip(AUDIT_DATA.keys(), tabs[:5])):
            with tab:
                ...
                if key == "Thermal":
                    ...
                    st.write("**Refresh Calculation:** At $T_{case} > 85C$, $tREFI$ scales from $7.8\mu s$ to $3.9\mu s$.")
        ...
        with tabs[5]:
            ...
            if st.button("Download Full Audit Report"):
                ...
                pdf.add_page()
                # Iterate through all sections to populate the PDF
                for title, content in AUDIT_DATA.items():
                    pdf.add_sec(title, content['about'], content['df'])
                
                pdf.add_page()
                pdf.set_font('Arial', 'B', 14); pdf.cell(0, 10, "Engineering Solutions", 0, 1, 'L')
