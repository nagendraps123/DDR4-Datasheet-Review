...
# Safe references
s_ref = JEDEC_MASTER['SPEED'][default_bin]
d_ref = JEDEC_MASTER['DENSITY'][default_density]
...
with tabs[1]:
    ...
    st.table([
        ...
        {"Parameter": "Row Addressing", "Value": default_density, "JEDEC": d_ref['Rows'], "Source": "🟢 Extracted"},
...
with tabs[2]:
    ...
    st.markdown("**Reviewer Context:** Verify voltage tolerance under high-speed operation and power-up sequencing.")

# --- 6d. AC Timings ---
with tabs[3]:
    st.subheader("Tab 4: AC Timings")
    v_taa = 14.06  # sample measured value
    ...
    st.table([
        ...
        {"Parameter": "tRP", "Value": "13.75ns", "JEDEC": f"{s_ref['tRP']}ns", "Source": "🟢 Extracted", "Status": "✅ PASS"},
        {"Parameter": "tRAS", "Value": "32ns", "JEDEC": f"{s_ref['tRAS']}ns", "Source": "🟢 Extracted", "Status": "✅ PASS"}
    ])
...
with tabs[4]:
    ...
    st.markdown("**Reviewer Context:** Ensure refresh cycles do not exceed memory bandwidth budget.")
...
- **Reviewer Context:** Misaligned training can cause SI-related errors under load
""")
...
- SI risk indicators
- **Reviewer Context:** Critical for high-speed DDR; check PCB routing, termination, and decoupling
""")
...
- **Reviewer Context:** Validate operating margin across full JEDEC temperature range
""")
...
- **Reviewer Context:** Critical for system integrators to identify latent defects
""")
...
- Highlights DDR4 improvements: more banks, higher bandwidth, lower voltage
- Why DDR3 limits do not apply
- **Reviewer Context:** Guides backwards compatibility and migration planning
""")

# --- 6k. Review Summary & Scorecard ---
with tabs[10]:
